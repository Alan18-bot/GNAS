...
with strategy.scope():
  def create_weight(name, shape, initializer=None, trainable=True, seed=None):
    ...
    w = tf.get_variable(name, shape, initializer=initializer, trainable=trainable)#,aggregation=tf.VariableAggregation.MEAN)
  ...
  def batch_norm(x, is_training, name="bn", decay=0.9, epsilon=1e-5,
      ...
      with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
          offset = tf.get_variable(
              "offset", shape,
              initializer=tf.constant_initializer(0.0, dtype=tf.float32))#, aggregation=tf.VariableAggregation.MEAN)
          scale = tf.get_variable(
              "scale", shape,
              initializer=tf.constant_initializer(1.0, dtype=tf.float32))#, aggregation=tf.VariableAggregation.MEAN)
      ...
      with strategy.scope():
          ...
          def build_train_single(data_input):
            with tf.GradientTape() as tape:
              ...
              def _pre_process(x):
                  x = tf.pad(x, [[28, 28], [28, 28], [0, 0]])
                  x = tf.random_crop(x, [224, 224, 3], seed=child_model.seed)
                  x = tf.image.random_flip_left_right(x, seed=child_model.seed)
                  if child_model.cutout_size is not None:
                      mask = tf.ones([child_model.cutout_size, child_model.cutout_size], dtype=tf.int32)
                      start = tf.random_uniform([2], minval=0, maxval=224, dtype=tf.int32)
                      mask = tf.pad(mask, [[child_model.cutout_size + start[0], 224 - start[0]],
                                           [child_model.cutout_size + start[1], 224 - start[1]]])
                      mask = mask[child_model.cutout_size: child_model.cutout_size + 224,
                             child_model.cutout_size: child_model.cutout_size + 224]
                      mask = tf.reshape(mask, [224, 224, 1])
                      mask = tf.tile(mask, [1, 1, 3])
                      x = tf.where(tf.equal(mask, 0), x=x, y=tf.zeros_like(x))
                  if child_model.data_format == "NCHW":
            ...
            with tf.variable_scope(child_model.name, reuse=tf.AUTO_REUSE):
                learning_rate = tf.train.cosine_decay_restarts(learning_rate=child_model.lr_max, global_step=child_model.global_step,
                                                               first_decay_steps=child_model.lr_T_0 * child_model.num_train_batches,
                                                               t_mul=child_model.lr_T_mul, alpha=child_model.lr_min)
          ...
          def build_train():
                  ...
                  child_model.top_k_train_acc = strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_top_k_acc, axis=None)
          ...
          def build_test():
                  ...
                  child_model.top_k_test_acc = strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_top_k_acc, axis=None)
...
class Model(object):
      ...
      def _maybe_calibrate_size_new_giveup(self, layers, out_filters, is_training):
          ...
          with tf.variable_scope("calibrate"):
              ...
              if hw[0] != hw[1]:
                  ...
                  with tf.variable_scope("pool_x"):
                      w_depthwise = create_weight("w_depth", [1, 1, c[0], 1])
                      w_pointwise = create_weight("w_point", [1, 1, c[0], out_filters])
                      x = tf.nn.relu(x)
                      H = self._get_HW(x)
                      compute_sep_conv_macc(1,c[0],out_filters,H)
                      x = tf.nn.separable_conv2d(
                          x,
                          depthwise_filter=w_depthwise,
                          pointwise_filter=w_pointwise,
                          strides=[1,1,1,1], padding="SAME", data_format=self.data_format)
                      x = batch_norm(x, is_training, data_format=self.data_format)
              ...
              if c[1] != out_filters:
                  ...
                  with tf.variable_scope("pool_y"):
                      w_depthwise = create_weight("w_depth", [1, 1, c[1], 1])
                      w_pointwise = create_weight("w_point", [1, 1, c[1], out_filters])
                      y = tf.nn.relu(y)
                      H = self._get_HW(y)
                      compute_sep_conv_macc(1, c[1], out_filters, H)
      ...
      def _model(self, o_images, is_training, reuse=tf.AUTO_REUSE):
              ...
              with g1.as_default() as graph:
                  with tf.variable_scope(self.name, reuse=reuse):
                      ...
                      # images = slim.batch_norm(o_images)
                      images = o_images
                      # self.xx = tf.identity(images)

                      # fake_input = tf.placeholder(tf.float32, [1, 224, 224, 3], 'input')
                  ...
                  tf.logging.info('conv macc = {0}, sep_conv macc = {1}, fc macc = {2}'.format(MACC[0] // self.num_gpus,
                                                                                               MACC[1] // self.num_gpus,
                                                                                               MACC[2] // self.num_gpus))
                  tf.logging.info('sum macc = {}'.format(sum(MACC) // self.num_gpus))
      ...
      def _fixed_layer_orginal(self, layer_id, prev_layers, arc, out_filters, stride,
