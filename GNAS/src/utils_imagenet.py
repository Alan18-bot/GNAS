...
with strategy.scope():
    ...
    moving_average=None):
      ...
      if lr_cosine:
          ...
          learning_rate = tf.train.cosine_decay_restarts(learning_rate=lr_max,global_step=train_step,first_decay_steps=lr_T_0*num_train_batches,t_mul=lr_T_mul,alpha=lr_min)
...
def count_model_params(tf_variables):
  ...
  for var in tf_variables:
    num_vars += np.prod([dim for dim in var.get_shape()]) #value
  return num_vars
